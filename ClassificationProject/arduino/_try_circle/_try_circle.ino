...
bool readmode = false;
float x = 11, y = -10;
float ra = sqrt(pow(x-3, 2) + pow(y-3, 2)), de =  atan((y-3) / float(x-3));
...
void locate(float distance, float height, float alpha) {
  ...
  float len =
  pwn0 = thetaToPw sqrt(pow(distance, 2) + pow(height, 2));
  float jiaodu = atan(height / distance);
  int pwn0, pwn1, pwn2, pwn3;
n((90)/180*PI);
  pwn1 = thetaToPwn((0)/180*PI);
  pwn2 = thetaToPwn((0)/180*PI);
  pwn3 = thetaToPwn((0)/180*PI);
//  pwn1 = thetaToPwn(PI / 2 - jiaodu - acos((25 * pow(len, 2) - 6196) / float(40 * (13 * len + 200))));
//  pwn2 = thetaToPwn(acos((25 * pow(len, 2) + 1196) / float(400 * (2 * len + 13))));
//  pwn3 = thetaToPwn(acos((25 * pow(len, 2) - 6196) / float(40 * (13 * len + 200))) ) + float(6) / 180.0 * PI;
